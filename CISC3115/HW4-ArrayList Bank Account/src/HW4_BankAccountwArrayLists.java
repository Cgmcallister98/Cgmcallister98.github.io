...
public class HW4_BankAccountwArrayLists {
	...
	public static void printAccts(Bank bank, PrintWriter outFile) {
		...
		for (int index = 0; index < bank.getNumAccounts(); index++) {
			...
			if(myAccount.getStatus() == true)
				status = "Open"; //assign status
			else status = "Closed"; 
			outFile.printf("%8s", status);
			if(bank.getAccount(index).getAcctType().contentEquals("CD")) {
			outFile.printf("%4d",myAccount.getDateInfo().getMonth()+1);
			outFile.printf("/%1d/",myAccount.getDateInfo().getDayOfMonth());
			outFile.printf("%2d",myAccount.getDateInfo().getYear());
			outFile.println();
			}
	...
	public static void deposit(Bank bank, PrintWriter outFile, Scanner kybd) {
		...
		if (index == -1) // invalid account
		{
			...
			outFile.println(trans.getFailure());
		} else if(bank.getAccount(index).getStatus() == false) {
			...
			if(today.compareDate(bank.getAccount(index).getDateInfo()) == 0) {
				...
				if(newDate == 6) {//assign new maturity date
		...
		else{  	// prompt for amount to deposit-non CD
	...
	public static void withdrawal(Bank bank, PrintWriter outFile, Scanner kybd) {
		...
		if (index == -1) // invalid account
		{
			...
			if(today.compareDate(bank.getAccount(index).getDateInfo()) == 0) {
				...
				if(bank.getAccount(index).makeWithdrawal(amountToWithdraw) == -1) {
		...
		{// prompt for amount to Withdraw
			...
			if(withdrawReport == 0) {//invalid amount
				trans.setTransType(str);
				trans.setIndicator(false);
				trans.setTransDate(today);
				trans.setFailure(2);
			...
			else if(withdrawReport == -1) {//insufficient funds
				trans.setTransType(str);
				trans.setIndicator(false);
				trans.setTransDate(today);
				trans.setFailure(3);
				bank.getAccount(index).addTransaction(trans);
				outFile.println(trans.getFailure());
			} else if(withdrawReport == 1) {//valid withdrawal
				trans.setTransType(str);
				trans.setIndicator(true);
				trans.setTransDate(today);
				trans.setTransAmount(amountToWithdraw);
	...
	public static void clearCheck(Bank bank, PrintWriter outFile, Scanner kybd) {
		...
		if (index == -1) // invalid account
		{
			...
			trans.setTransType(str);//closed acount
			trans.setIndicator(false);
			trans.setTransDate(today);
			trans.setFailure(0);
			bank.getAccount(index).addTransaction(trans);
			...
			if (today.clearCheckDate(dateToClear) == 1) {
				...
				if(clearResult == 0) {//check bounced
					trans.setTransType(str);
					trans.setIndicator(false);
					trans.setTransDate(today);
					trans.setFailure(4);
					...
					outFile.printf("Balance: $%.2f", 
							bank.getAccount(index).getBalance());
					outFile.println();
				}else if(clearResult == -1) {//check bounced 
					trans.setTransType(str);
					trans.setIndicator(false);
					trans.setTransDate(today);
					trans.setFailure(4);
					...
					outFile.printf("New Balance: $%.2f", 
							bank.getAccount(index).getBalance());
					outFile.println();
				} else if(clearResult == 1){//check cleared
					trans.setTransType(str);
					trans.setIndicator(true);
					trans.setTransDate(today);
	...
	public static void acctInfo(Bank bank, PrintWriter outFile, Scanner kybd) {
		...
		if (bank.getAccount(index).getDepositor().getSSNumber()
			...
			outFile.println("Account Holder: " + 
					bank.getAccount(index).getDepositor().getName().getFirst() +" "+
					bank.getAccount(index).getDepositor().getName().getLast());
			outFile.println("Account Holder SSID: " + 
					bank.getAccount(index).getDepositor().getSSNumber());
	...
	public static void acctHistory(Bank bank, PrintWriter outFile, Scanner kybd) {
		...
		if (bank.getAccount(index).getDepositor().getSSNumber().
				...
				outFile.println("Transaction Requested: Account Info with History");
					...
					bank.getAccount(index).getAcctNum());
					outFile.println("Account Type: " + 
					bank.getAccount(index).getAcctType());
			...
			for (int i = 0; i < bank.getAccount(index).getTransactionSize();i++) {
						outFile.println();
						outFile.println("Transaction: " + 
						bank.getAccount(index).getTransactions(i).getTransType());
			if (bank.getAccount(index).getTransactions(i).getIndicator() == false) {
			outFile.println(bank.getAccount(index).getTransactions(i).getFailure());
			}else if(bank.getAccount(index).getTransactions(i).getTransType()
					.contentEquals("New Account")) {
			outFile.printf("Date: %2d/%1d/%-4d ", 
			bank.getAccount(index).getTransactions(i).getTransDate().getMonth()+1, 
		   bank.getAccount(index).getTransactions(i).getTransDate().getDayOfMonth(), 
			bank.getAccount(index).getTransactions(i).getTransDate().getYear());
			outFile.println();
			}else {
			outFile.println("Amount: " +
			bank.getAccount(index).getTransactions(i).getTransAmount());
			outFile.printf("Date: %2d/%1d/%-4d ", 
			bank.getAccount(index).getTransactions(i).getTransDate().getMonth()+1,
		   bank.getAccount(index).getTransactions(i).getTransDate().getDayOfMonth(), 
			bank.getAccount(index).getTransactions(i).getTransDate().getYear());
			outFile.println();
				...
				bank.getAccount(index).getBalance());
				outFile.println("Transaction History: ");
				outFile.println(bank.getAccount(index).
						getTransactions(0).getTransType());
			for (int i = 0; i < bank.getAccount(index).getTransactionSize();i++){
			outFile.println("Transaction: " + bank.getAccount(index).
					getTransactions(i).getTransType());
			if (bank.getAccount(index).getTransactions(i).getIndicator() == false) {
			outFile.println(bank.getAccount(index)
					.getTransactions(i).getFailure());
			}else if(bank.getAccount(index).getTransactions(i)
					.getTransType().contentEquals("New Account")) {
			outFile.printf("Date: %2d/%1d/%-4d ", 
			bank.getAccount(index).getTransactions(i).getTransDate().getMonth()+1, 
		   bank.getAccount(index).getTransactions(i).getTransDate().getDayOfMonth(), 
			bank.getAccount(index).getTransactions(i).getTransDate().getYear());
			...
			bank.getAccount(index).getTransactions(i).getTransAmount());
			outFile.printf("Date: %2d/%1d/%-4d ", 
			bank.getAccount(index).getTransactions(i).getTransDate().getMonth(), 
		   bank.getAccount(index).getTransactions(i).getTransDate().getDayOfMonth(), 
			bank.getAccount(index).getTransactions(i).getTransDate().getYear());
			outFile.println();
			}
