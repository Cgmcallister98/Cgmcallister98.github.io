...
public class Account {
	  ...
	  if(acctStatus == true) {
		if(acctType.contentEquals("CD")) {
			...
			addTransaction(trans);
				...
				acctBalance += depositAmount;
				Bank.addChecking(depositAmount);
		...
		if(acctType.equals("CD")) {
			if(today.compareDate(date) == 0) {
				...
				if(newDate == 6) {
					...
					Bank.subtractCD(amountToWithdraw);
			...
			return -2;
				...
				acctBalance -= amountToWithdraw;
				Bank.subtractSavings(amountToWithdraw);
				...
				acctBalance -= amountToWithdraw;
				Bank.subtractChecking(amountToWithdraw);
				trans.setTransType(str);
				trans.setIndicator(true);
				trans.setTransDate(today);
				trans.setTransAmount(amountToWithdraw);
	...
	public int clearCheck(double checkToClear, DateInfo dateToClear) { 
		 ...
		 if(acctStatus == true) {
			if(today.clearCheckDate(dateToClear) == 1){
				if(getBalance() < checkToClear && getBalance() < 2.5 ) {
					...
					Bank.subtractChecking(acctBalance);
		...
		trans.setTransType(str);//not checking account
		trans.setIndicator(false);
		trans.setTransDate(today);
		trans.setFailure(6);
		addTransaction(trans);
	...
	public Account getAccount() {
		Account acct = new Account(acctNum, acctType, acctStatus, acctBalance, depositor, date);
		return acct;
	}
